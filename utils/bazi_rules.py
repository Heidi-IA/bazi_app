...
# Lista de los 12 animales válidos, para poder reconocer el nombre del
# animal aunque la extracción por imagen lo haya guardado bajo una clave
# distinta a "nombre" dentro de "rama" (p. ej. "animal" o "signo") o con
# variaciones de mayúsculas/acentos/espacios.
_ANIMALES_VALIDOS = {
...
def _normalizar_texto(s):
    """NFC + strip, para comparar strings sin que espacios extra o
    normalizaciones Unicode distintas (NFC vs NFD) rompan una igualdad."""
...
def _sin_acentos(s):
    """Quita acentos para poder comparar 'Dragón' con 'Dragon' sin que la
    tilde sea la diferencia entre detectar o no un conflicto."""
    nfkd = unicodedata.normalize("NFKD", s)
    return "".join(c for c in nfkd if not unicodedata.combining(c))


def _rama_nombre(datos):
    """Extrae el nombre del animal (rama) de un pilar de forma tolerante
    a variaciones de formato de la extracción por imagen:
    - Formato esperado: {"rama": {"nombre": "Tigre", ...}}
    - Tolera también: {"rama": {"animal": "Tigre", ...}},
      {"rama": {"signo": "Tigre", ...}}, o {"rama": "Tigre"} (string plano).
    - Normaliza espacios y Unicode para que la comparación con las tablas
      de reglas (CHOQUES_RAMAS, DANOS_RAMAS, etc., que usan nombres con
      tilde y mayúscula inicial) sea consistente.
    - Tolera que el valor venga en PINYIN (Zi, Chou, Yin, Mao, Chen, Si,
      Wu, Wei, Shen, You, Xu, Hai) en vez de en español, traduciéndolo
      al nombre canónico en español.
    Devuelve el nombre con capitalización estándar (p. ej. "Dragón") si
    reconoce el animal, o el string normalizado tal cual si no lo reconoce.
    """
    rama = datos.get("rama", {})
    candidato = ""
    if isinstance(rama, dict):
        for clave in ("nombre", "animal", "signo", "zodiaco"):
            valor = rama.get(clave, "")
            if valor:
                candidato = valor
                break
    elif isinstance(rama, str):
        candidato = rama
    ...
    clave_busqueda = _sin_acentos(candidato).lower()

    # 1) ¿Es directamente un nombre de animal en español?
    if clave_busqueda in _ANIMALES_VALIDOS:
        for nombre_canonico in ANIMALES_DESC.keys():
            if _sin_acentos(nombre_canonico).lower() == clave_busqueda:
                return nombre_canonico

    # 2) ¿Es el pinyin de la rama (Zi, Wu, Yin, Si, Chen, Wei, ...)?
    #    Ojo: algunos pinyin de rama coinciden en escritura con pinyin de
    #    tronco (p. ej. "Wu" es tanto Tierra Yang como el Caballo), pero
    #    acá solo se usa dentro del contexto de "rama", así que es seguro.
    for nombre_es, pinyin in RAMA_A_PINYIN.items():
        if _sin_acentos(pinyin).lower() == clave_busqueda:
            return nombre_es
...
def detectar_relaciones(pilares, dm_tronco, dm_rama, sexo, flor_melocoton_animal="", pilares_moviles=None):
    ...
    for p, datos in pilares.items():
        if _rama_nombre(datos) == flor_melocoton_animal:
    ...
    for nombre_pilar, datos in pilares.items():
        ...
        rama_nombre = _rama_nombre(datos)
