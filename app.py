...
PAUTAS GENERALES
═══════════════════════════════════
- Texto fluido y profesional, en español, con párrafos bien desarrollados
- Tono: cálido, profesional, orientado al crecimiento personal
- Sexo del consultante: {sexo} — usar pronombres correctos
- NO incluir tablas, solo texto fluido en párrafos (separados por una línea en blanco)
- Respondé ÚNICAMENTE con el texto de la sección: sin JSON, sin markdown, sin backticks,
  sin encabezados ni título (el número y el título de la sección ya se agregan aparte
  en el documento final, no los repitas)
- Si necesitás citar una frase o expresión textual, usá comillas simples (por ejemplo
  'hoy te amo, mañana te odio'). NO uses comillas dobles en ningún momento.
"""
...
═══════════════════════════════════
Escribí el texto de la sección "01 — El día maestro". Debe incluir:
...
═══════════════════════════════════
Escribí el texto de la sección "02 — Análisis de los 4 pilares".
...
═══════════════════════════════════
Escribí el texto de la sección "03 — Carta móvil".
...
═══════════════════════════════════
Escribí el texto de la sección "04 — Los 5 elementos — balance y finanzas".
...
═══════════════════════════════════
Escribí el texto de la sección "05 — Combinaciones".
...
═══════════════════════════════════
Escribí el texto de la sección "06 — Conflictos".
...
Recordá: si citás una frase textual (p. ej. el patrón psicológico de una destrucción),
usá comillas simples, nunca comillas dobles.
"""
...
═══════════════════════════════════
Escribí el texto de la sección "07 — Salud".
...
═══════════════════════════════════
Escribí el texto de la sección "08 — Relaciones".
...
═══════════════════════════════════
Escribí el texto de la sección "09 — Pilares de la suerte".
...
═══════════════════════════════════
Escribí el texto de la sección "10 — Recomendaciones finales".
...
def _llamar_claude_texto_seccion(prompt, max_tokens=4000):
    """Llama a Claude y devuelve el texto plano de la sección.

    Antes, cada sección se pedía envuelta en un JSON ({"secciones": [...]})
    y se parseaba con json.loads(). El problema es que el texto narrativo
    en español que redacta Claude puede incluir comillas, apóstrofes o
    saltos de línea, y para que eso conviva dentro de un string JSON hay
    que escaparlo perfectamente: una sola comilla doble o salto de línea
    mal escapado en la respuesta del modelo rompía el parseo (json.loads
    tira "Expecting ',' delimiter") y tiraba abajo la sección entera,
    quedando el placeholder de error en su lugar (esto es justo lo que le
    pasó a la sección 06 de Conflictos: es la más larga y la que más cita
    frases textuales, como el patrón psicológico de una destrucción).
    Como numero y titulo ya los conoce quien llama a esta función, no hace
    falta pedirle a Claude que los repita dentro de un JSON: alcanza con
    pedirle el texto de la sección en texto plano, lo que elimina esta
    clase entera de errores.
    """
    response = client.messages.create(
        model="claude-sonnet-4-6",
    ...
    # Por si Claude igual envuelve la respuesta en una cerca de código o en
    # comillas, lo limpiamos con tolerancia (ya no dependemos de esto para
    # poder parsear, es solo prolijidad).
    texto = re.sub(r"^```[a-zA-Z]*\n?", "", texto)
    texto = re.sub(r"\n?```$", "", texto).strip()
    if response.stop_reason == "max_tokens":
        raise Exception(
            "La respuesta de Claude se cortó por el límite de tokens "
            "antes de terminar el texto. Subí max_tokens en "
            "_llamar_claude_texto_seccion o dividí el prompt en partes más chicas."
        )
    return texto
...
def _generar_seccion_con_reintento(datos_carta, instrucciones, pautas, numero, titulo):
    ...
    for intento, tokens in enumerate([4096, 6000], start=1):
        try:
            texto = _llamar_claude_texto_seccion(prompt, max_tokens=tokens)
            return [{"numero": numero, "titulo": titulo, "texto": texto}]
        except Exception as e:
            if intento == 2:
                return [{
                    "numero": numero,
